...
class Cart extends Model
{
    ...
    {
        $size_cost = 0;
        $storedItem = ['user_id' => $item->user_id, 'qty' => 0, 'product_city' => $item->product_city, 'size_key' => 0, 'size_qty' => $item->size_qty, 'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0', 'keys' => $keys, 'values' => $values, 'item_price' => $item->price, 'discount' => 0, 'affilate_user' => 0];
        ...
        $item_price += $size_cost;
        $storedItem['item_price'] = $item_price;
        if (! empty($item->whole_sell_qty)) {
            ...
            if (Session::has('current_discount')) {
                ...
                if (array_key_exists($id.$size.$color.str_replace(str_split(' ,'), '', $values), $data)) {
                    $discount = $item_price * ($data[$id.$size.$color.str_replace(str_split(' ,'), '', $values)] / 100);
    ...
    {
        $size_cost = 0;
        $storedItem = ['user_id' => $item->user_id, 'qty' => 0, 'size_key' => 0, 'size_qty' => $item->size_qty, 'product_city' => $item->product_city, 'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0', 'keys' => $keys, 'values' => $values, 'item_price' => $item->price, 'discount' => 0, 'affilate_user' => 0];
        if ($item->type == 'Physical') {
            if ($this->items) {
                if (array_key_exists($id.$size.$color.str_replace(str_split(' ,'), '', $values), $this->items)) {
                    $storedItem = $this->items[$id.$size.$color.str_replace(str_split(' ,'), '', $values)];
        ...
        $this->items[$id.$size.$color.str_replace(str_split(' ,'), '', $values)] = $storedItem;
        $this->totalQty += $qty;
        $this->totalPrice += ($item_price * $qty);
    }

    public function adding($item, $id, $size_qty, $size_price)
    {
        $storedItem = ['user_id' => $item->user_id, 'qty' => 0, 'size_key' => 0, 'product_city' => $item->product_city, 'size_qty' => $item->size_qty, 'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0', 'keys' => '', 'values' => '', 'item_price' => $item->price, 'discount' => 0, 'affilate_user' => 0];
        if ($this->items) {
        ...
        if (! empty($item->whole_sell_qty)) {
            ...
            if (Session::has('current_discount')) {
                ...
                if (array_key_exists($id, $data)) {
                    $discount = $item_price * ($data[$id] / 100);
                    $item_price = $item_price - $discount;
                }
            }
        }
