...
class IncomeController extends Controller
{
    ...
    $last_30_days = Order::whereBetween('created_at', [$last30days, $current_date])
        ->where('commission', '!=', 0);
    $current_month = Order::whereBetween('created_at', [$first_day, $current_date])
        ->where('commission', '!=', 0);
    $sign = Currency::where('is_default', '=', 1)->first();
    if ($request->start_date && $request->end_date) {
        $start_date = Carbon::parse($request->start_date);
    ...
    return view('admin.earning.commission_earning', [
        'orders' => $orders->count() > 0 ? $orders->get() : [],
        'total' => $orders->count() > 0 ? $sign->sign . $orders->sum('commission') : 0,
        'start_date' => isset($start_date) ? $start_date : '',
        'end_date' => isset($end_date) ? $end_date : '',
        'currency' => $sign,
