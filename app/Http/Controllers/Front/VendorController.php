...
class VendorController extends FrontBaseController
{
    public function index(Request $request, $slug)
    {
        ...
        if (empty($vendor)) {
            ...
            // Milestone 1 Fix: Handle 301 Redirects for malformed slugs (e.g., 'anti%20scam' -> 'anti-scam')
            $decodedSlug = str_replace(' ', '-', urldecode($slug));
            if ($decodedSlug !== $slug) {
                return redirect()->to(url($decodedSlug), 301);
            }

            $page = DB::table('pages')->where('slug', $slug)->first();

            // Try case-insensitive fallback for pages like /Privacy
            if (empty($page)) {
                $page = DB::table('pages')->where('slug', 'LIKE', $slug)->first();
                if ($page) {
                    return redirect()->to(url($page->slug), 301);
                }
            }
