                    ...
                    <div class="product-description">
                      ...
                      <form id="geniusformdata" action="{{route('admin-cat-update',$data->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-7">
                            <select class="input-field" name="category_type" required="">
                              <option value="physical" {{ $data->category_type == 'physical' ? 'selected' : '' }}>{{ __('Physical') }}</option>
                              <option value="digital" {{ $data->category_type == 'digital' ? 'selected' : '' }}>{{ __('Digital') }}</option>
                              <option value="license" {{ $data->category_type == 'license' ? 'selected' : '' }}>{{ __('License') }}</option>
                              <option value="listing" {{ $data->category_type == 'listing' ? 'selected' : '' }}>{{ __('Listing') }}</option>
                            </select>
                          </div>
                        </div>
						...
						<div class="row">
                          ...
                          <div class="col-lg-7">
                            <div class="img-upload">
                              <div id="image-preview" class="img-preview" style="background: url({{ $data->image ? asset('assets/images/categories/'.$data->photo):asset('assets/images/noimage.png') }});">
                                <label for="image-upload" class="img-label"><i class="icofont-upload-alt"></i>{{ __('Upload Image') }}</label>
