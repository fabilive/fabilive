               ...
               <div class="col-lg-6">
                  <div class="widget border-0 p-30 widget_categories bg-light account-info">
                     ...
                     <div class="user-info">
                        ...
                        <h5 class="title w-price mb-3">{{ $sign->sign }}{{ number_format($user->balance, 2) }}</h5>

                        <!--<h5 class="title w-price mb-3">{{ App\Models\Product::vendorConvertPrice(Auth::user()->balance) }}</h5> -->
                        <!--<a href="{{ route('user-deposit-create') }}" class="mybtn1 sm "> <i class="fas fa-plus"></i>-->
                        <!--     {{ __('Add Deposit') }}</a>-->
                     </div>
                  </div>
               </div>
                           ...
                           <tbody>
                              ...
                              <tr>
                                 ...
                                 <td data-label="{{ __('View') }}">
                                    <div>
                                       <a class="mybtn1 sm1 mb-1" href="{{route('user-order',$order->id)}}">
                                          {{ __('View Order') }}
                                       </a>

                                       {{-- @if(
                                        $order->deliveryRider &&
                                        in_array($order->deliveryRider->status, ['accepted', 'delivered'])
                                    )
                                        <a href="{{ route('user-messages-delivery', [
                                                'order_id'    => $order->id,
