    ...
    <div class="full-row bg-light overlay-dark py-5"
        ...
        <div class="container">
            <div class="row text-center text-white">
                <div class="col-12">
                    <h3 class="mb-2 text-white">{{ __('Chat With Buyer') }}</h3>
                </div>
                <div class="col-12">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 d-inline-flex bg-transparent p-0">
                            <li class="breadcrumb-item">
                                <a href="{{ route('rider-dashboard') }}">{{ __('Dashboard') }}</a>
                            </li>
                            <li class="breadcrumb-item active">{{ __('Chat With Buyer') }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
                            ...
                            <ul class="list-group list-group-flush" id="chat-list">
                                @forelse($chats as $chat)
                                    <li class="list-group-item list-group-item-action chat-item
                                        ...
                                        <strong class="p-1">{{ $chat->buyer->name ?? 'Unknown Buyer' }}</strong>
...
chatHeader.innerText = `Product: ${productName}`;
                ...
                fetch("{{ route('rider-chat-messages') }}", {
                        method: "POST",
                        headers: {
        ...
        function renderMessages(messages) {
            ...
            messages.forEach(msg => {
                const isRider = msg.sender_id == {{ auth()->guard('rider')->id() }};
