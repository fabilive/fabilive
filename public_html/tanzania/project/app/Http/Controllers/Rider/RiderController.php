...
class RiderController extends RiderBaseController
{
    ...
    public function serviceAreaStore(Request $request)
    {
        ...
        $service_area->service_area_id = $request->service_area_id; // ✅ correctly named now
        $service_area->save();
    ...
    {
        $rules = [
            'service_area_id' => 'required|unique:rider_service_areas,service_area_id,' . $id . ',id,rider_id,' . $this->rider->id . '|exists:service_areas,id',
        ];
        ...
        $service_area = RiderServiceArea::findOrFail($id);
        $service_area->rider_id = $this->rider->id;
        $service_area->service_area_id = $request->service_area_id; // ✅ corrected
