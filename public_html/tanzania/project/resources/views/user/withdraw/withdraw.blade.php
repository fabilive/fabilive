                  ...
                  <div class="widget border-0 p-40 widget_categories bg-light account-info">
                     ...
                     <form id="userform" class="form-horizontal" action="{{route('user-wwt-store')}}" method="POST"
                        ...
                        <div class="form-group mb-4">
                           <label class="control-label col-sm-4" for="name">{{ __('Affiliate Income Balance') }}
                        ...
                        <div class="" id="paypal" style="display: none;">
                           <div class="form-group">
                            ...
                            <div id="pesapal" style="display: none;">
                                ...
                                <div class="item form-group">
                                    <label class="control-label col-sm-12" for="pesapal_acc_no">{{ __('Account Number') }} *</label>
                                    <div class="col-sm-12">
                                        <input name="pesapal_acc_no" placeholder="{{ __('Enter Account Number') }}" class="form-control" type="text">
                                    </div>
                                </div>
                                <div class="item form-group">
                                    <label class="control-label col-sm-12" for="pesapal_acc_name">{{ __('Account Name') }} *</label>
                                    <div class="col-sm-12">
...
(function($) {
    ...
    $("#withmethod").change(function () {
        ...
        if (method == "Bank") {
            ...
            $("#paypal, #pesapal").find('input, select').attr('required', false);
        } else if (method == "Pesapal") {
            $("#pesapal").show();
            $("#bank, #paypal").hide();
            $("#pesapal").find('input, select').attr('required', true);
            $("#bank, #paypal").find('input, select').attr('required', false);
        } else if (method != "") {
            $("#paypal").show();
            $("#bank, #pesapal").hide();
            $("#paypal").find('input').attr('required', true);
            $("#bank, #pesapal").find('input, select').attr('required', false);
        } else {
            $("#bank, #paypal, #pesapal").hide();
            $("#bank, #paypal, #pesapal").find('input, select').attr('required', false);
        }
    });
})(jQuery);
</script>
