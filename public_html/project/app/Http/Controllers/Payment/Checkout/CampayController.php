    ...
    public function store(Request $request)
{
    ...
    try {
        // 2️⃣ Prepare cart & total
        $oldCart = Session::get('cart');
        $cart = new Cart($oldCart);
        $currency = $this->curr->name; // e.g., XAF
        $total = $request->total / $this->curr->value;
        $total = $total * $this->curr->value;

        // 3️⃣ Generate Campay payment link
        $response = $this->campayService->generatePaymentLink(
            $currency,
            round($total / $cart->totalQty),
            'This is the live payment',
            route('front.campay.notify'),
            [
        ...
        // 4️⃣ Check response & redirect
        if (isset($response['link'])) {
            Session::put('input_data', $input);                 // Store input data for callback if needed
            Session::put('campay_reference', $response['reference']); // Store Campay reference
            return redirect()->away($response['link']);        // Redirect to Campay payment
        }
    ...
    public function notify(Request $request)
    {
        ...
        if (strtolower($request->status) === 'successful') {
            ...
            $fixedCommission = 5; // 5 currency units
            $percentageCommission = 5; // 5 percent
            $cartTotal = $cart->totalPrice; // Get total cart price
            $input['commission'] = $fixedCommission + ($percentageCommission / 100 * $cartTotal);
            $input['service_area_id'] = $input['service_area_id'] ?? null;
